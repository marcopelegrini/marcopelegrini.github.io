...
A few metal places to give it a more premium feel and prevent it from easily slip through your table.
...
For instance, if you want be bottom-left key (currenty `KC_P1`) to function as "copy":
Replace `KC_P1` for `LCMD(KC_C)` on macos or `LCTL(KC_C)` on Windows.
